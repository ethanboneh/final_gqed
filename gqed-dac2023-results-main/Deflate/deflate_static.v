...
begin: MYHDL77_RETURN
    MYHDL7_get4 = ((b41 >>> (dio + boffset)) & ((1 << width) - 1));
end
endfunction
...
function integer MYHDL10_get4;
    input boffset;
    integer boffset;
    input width;
    integer width;
begin: MYHDL80_RETURN
    MYHDL10_get4 = ($signed($signed({1'b0, b41}) >>> ($signed({1'b0, dio}) + boffset)) & ((1 << width) - 1));
end
endfunction
...
begin: MYHDL83_RETURN
    ...
    do_variable <= (do_variable + pshift);
    o_oprogress <= (do_variable + pshift);
...
task MYHDL14_do_flush;
begin: MYHDL84_RETURN
    flush <= 1'b0;
    ob1 <= 0;
    o_oprogress <= (do_variable + 1);
...
function integer MYHDL19_rev_bits;
    input [24-1:0] b;
    input nb;
    integer nb;
    integer r;
...
begin: MYHDL111_RETURN
    MYHDL55_get_bits = ($signed({1'b0, aleaf}) & ((1 << 4) - 1));
end
endfunction
        ...
        case (state)
                    ...
                    if (1'b0) begin
                        ...
                        case (hm)
                            'h2: begin
                                ...
                                numCodeLength <= 0;
                                numLiterals <= 0;
                                static_variable <= 1'b0;
                                MYHDL8_adv(3);
                            end
                            'h1: begin
                                static_variable <= 1'b1;
                                cur_static <= 0;
            ...
            5'b11010: begin
                if ((!1'b0)) begin
                    ...
                    case (lencode)
                        ...
                        default: outbits = 227;
                    endcase
                    MYHDL17_put(outbits, outlen);
                    cur_i <= 0;
                end
                else if (outcarrybits) begin
                    MYHDL18_put(outcarry, outcarrybits);
                    state <= 5'b11011;
                end
                else begin
                    case ((cur_i + 1))
                    ...
                    if ((nextdist > cur_dist)) begin
                        ...
                        outcode = (MYHDL19_rev_bits(cur_i, 5) | (extra_dist << 5));
                        if ((extra_bits <= 4)) begin
                            MYHDL20_put(outcode, (5 + extra_bits));
                            state <= 5'b11011;
                        end
                        else begin
            ...
            5'b10010: begin
                if ((1'b1 && 1'b0)) begin
                    if (((method == 4) && 1'b0)) begin
                        dlwaddr <= spread;
                        dwleaf <= MYHDL52_makeLeaf(spread_i, codeLength[spread_i]);
                    end
                    else begin
                        lwaddr <= spread;
            ...
            5'b10011: begin
                if ((!1'b1)) begin
                ...
                else if ((1'b0 && (MYHDL55_get_bits(rleaf) >= cur_next))) begin
                    ...
                    if ((MYHDL57_get_bits(the_leaf) < 1)) begin
                    end
                    MYHDL59_adv(MYHDL58_get_bits(the_leaf));
                    code <= MYHDL60_get_code(the_leaf);
                    if ((1'b0 && (method == 2))) begin
                        state <= 5'b00011;
                    end
                    else begin
                        state <= 5'b10100;
            ...
            5'b01111: begin
                ...
                else if ((MYHDL62_get_bits(drleaf) >= cur_next)) begin
                    token = (code - 257);
                    case (token)
                        0: extraLength = 0;
                        1: extraLength = 0;
            ...
            5'b10000: begin
                if ((1'b1 && 1'b0)) begin
                    if ((MYHDL64_get_bits(drleaf) == 0)) begin
                    end
                    token = (code - 257);
                    case (token)
                        0: tlength = 3;
                    ...
                    distanceCode = MYHDL66_get_code(drleaf);
                    case (distanceCode)
                        0: distance = 1;
                        1: distance = 2;
                        2: distance = 3;
                    ...
                    case ($signed(distanceCode >>> 1))
                        ...
                        default: moreBits = 13;
                    endcase
                    mored = MYHDL68_get4((extraLength + MYHDL67_get_bits(drleaf)), moreBits);
                    distance = distance + mored;
                    if ((distance > $signed({1'b0, do_variable}))) begin
                    end
                    MYHDL70_adv(((moreBits + extraLength) + MYHDL69_get_bits(drleaf)));
                    offset <= (($signed({1'b0, do_variable}) - distance) & 511);
                    length <= tlength;
                    cur_i <= 0;
                    oraddr <= ($signed({1'b0, do_variable}) - distance);
                    state <= 5'b10101;
                end
            end
            5'b10100: begin
                ...
                else if ((do_variable >= (i_raddr + 512))) begin
                    // pass
                end
            ...
            5'b10101: begin
                if ((!1'b1)) begin
                ...
                else if (((cur_i == 0) && ((do_variable + length) >= (i_raddr + 512)))) begin
                    // pass
                end
                    ...
                    else if (((!1'b0) && (!final_variable))) begin
                        di <= (di + 2);
                        o_iprogress <= di;
                        state <= 5'b00001;
                        filled <= 1'b0;
                    ...
                    oraddr <= (offset + cur_i);
                    if ((cur_i == 1)) begin
                        off1 <= ($signed({1'b0, offset}) == (($signed({1'b0, do_variable}) - 1) & 511));
                        off2 <= ($signed({1'b0, offset}) == (($signed({1'b0, do_variable}) - 2) & 511));
                        copy1 <= orbyte;
                    end
                    if ((cur_i == 3)) begin
                        copy2 <= orbyte;
